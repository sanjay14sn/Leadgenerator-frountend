...
const PREDEFINED_TEMPLATES = [
  {
    ...
    items: [
      ...
      {
        id: 3,
        description: "Backend Setup (Node.js)",
        quantity: 1,
        price: 2500,
      },
  ...
  },
  {
    ...
    items: [
      ...
      {
        id: 3,
        description: "Content Creation (4 Blogs)",
        quantity: 1,
        price: 800,
      },
  ...
  },
  {
    ...
    items: [
      ...
      {
        id: 2,
        description: "Graphics Creation (12 Posts)",
        quantity: 1,
        price: 1200,
      },
      ...
      <div className="bg-white w-full max-w-6xl h-[90vh] rounded-2xl shadow-2xl flex overflow-hidden">
        ...
        <div className="w-80 bg-gray-50 border-r border-gray-200 flex flex-col">
          <div className="p-5 border-b border-gray-100">
            ...
            <p className="text-xs text-gray-500 mt-1">
              Customize your quote appearance
            </p>
          </div>
          ...
          <div className="flex-1 overflow-y-auto p-5 space-y-8">
              ...
              >
                ...
                {brand.logo ? (
                  ...
                  <div className="space-y-2">
                    <Upload
                      size={20}
                      className="mx-auto text-gray-400 group-hover:text-indigo-500 transition"
                    />
            ...
            <div className="space-y-3">
              ...
              >
                ...
                {brand.banner ? (
                  <div
                    className="h-20 w-full bg-cover bg-center rounded-lg"
        ...
        <div className="flex-1 bg-gray-100 relative overflow-hidden flex flex-col">
          ...
          <div className="flex-1 overflow-y-auto p-8 flex justify-center">
            ...
            <div
              className="bg-white w-full max-w-[210mm] min-h-[297mm] shadow-2xl flex flex-col text-sm text-gray-800 relative transition-all duration-300"
              ...
              <div className="p-12 flex-1 flex flex-col">
                ...
                <div className="flex justify-between items-start mb-16">
                  ...
                  <div className="text-right space-y-2">
                    <div>
                      <span className="text-gray-400 text-xs uppercase tracking-wider mr-4">
                        Date
                      </span>
                      <span className="font-medium">{clientDetails.date}</span>
                    </div>
                    <div>
                      <span className="text-gray-400 text-xs uppercase tracking-wider mr-4">
                        Valid Until
                    ...
                    <div>
                      ...
                      <span
                        className="font-bold px-2 py-0.5 rounded text-xs bg-gray-100 text-gray-600"
                        style={{
                          color: brand.color,
                ...
                <div className="mb-8">
                  <div
                    className="flex text-white font-bold p-3 rounded-t-lg"
                  ...
                  <div>
                    {items.map((item, i) => (
                      <div
                        key={item.id}
                        className={`flex p-3 border-b border-gray-100 ${i % 2 !== 0 ? "bg-gray-50" : ""}`}
                      >
                        ...
                        <div className="w-28 text-right text-gray-600">
                          {quoteDetails.currency} {item.price.toFixed(2)}
                        </div>
                ...
                <div className="flex justify-end mb-16">
                  <div className="w-1/2 space-y-3">
                    <div className="flex justify-between text-gray-500">
                      <span>Subtotal</span>
                      <span>
                        {quoteDetails.currency} {subtotal.toFixed(2)}
                      </span>
                    </div>
                    {discountAmount > 0 && (
                      <div className="flex justify-between text-red-500">
                        <span>Discount</span>
                        <span>
                          - {quoteDetails.currency} {discountAmount.toFixed(2)}
                        </span>
                      </div>
                    )}
                    <div className="flex justify-between text-gray-500">
                      <span>
                        {quoteDetails.taxLabel} ({quoteDetails.taxRate}%)
                      </span>
                      <span>
                        {quoteDetails.currency} {taxAmount.toFixed(2)}
                      </span>
                    </div>
                    ...
                    >
                      <span>Total</span>
                      <span>
                        {quoteDetails.currency} {total.toFixed(2)}
                      </span>
                    </div>
                  </div>
                </div>
                ...
                <div className="mt-auto space-y-8">
                  ...
                  {quoteDetails.terms && (
                    <div>
                      ...
                      <p
                        className="text-gray-500 text-xs whitespace-pre-line border-l-4 pl-4"
...
export default function SmartQuotes() {
  ...
  const [quoteDetails, setQuoteDetails] = useState({
    ...
    notes: "",
    terms:
      "Payment is due within 15 days of invoice date. Late payments are subject to a 5% fee.",
  });
  ...
  const discountAmount =
    quoteDetails.discountType === "percent"
  ...
  const loadTemplate = (templateId) => {
    ...
    if (window.confirm("This will replace your current items. Continue?")) {
      setItems(
        template.items.map((i) => ({ ...i, id: Date.now() + Math.random() })),
      );
      if (template.terms) {
  ...
  const generatePDF = () => {
    ...
    const primaryColor = hexToRgb(brand.color);
    ...
    const newPage = () => {
      ...
      doc.text(`Quote #${number} - Continued (Page ${currentPage})`, 20, 10);
    ...
    const writeLines = (text, x, width, font = 9, line = 5.5) => {
      doc.setFontSize(font);
      const lines = doc.splitTextToSize(text, width);
      for (const l of lines) {
        if (y + line > maxY) newPage();
        doc.text(l, x, y);
        y += line;
      }
    };
    ...
    if (clientDetails.address) {
      writeLines(clientDetails.address, 20, 80, 10, 5);
    }
    ...
    doc.setTextColor(0);
    doc.text("Date:", 140, rightY, { align: "right" });
    doc.text(clientDetails.date, 190, rightY, { align: "right" });

    doc.text("Valid Until:", 140, rightY + 6, { align: "right" });
    doc.text(clientDetails.dueDate, 190, rightY + 6, { align: "right" });

    doc.text("Status:", 140, rightY + 12, { align: "right" });
    doc.setTextColor(...primaryColor);
    doc.text(status, 190, rightY + 12, { align: "right" });

    y = Math.max(leftEndY, rightY + 20);
    ...
    items.forEach((item, i) => {
      const descLines = doc.splitTextToSize(item.description || "Item", 95);
      const rowHeight = Math.max(8, descLines.length * 5 + 2);
      ...
      // CENTER values vertically in row
      const centerY = y + rowHeight / 2 + 2;

      doc.text(String(item.quantity), 130, centerY, { align: "center" });
      doc.text(`${currency} ${item.price.toFixed(2)}`, 150, centerY, {
        align: "right",
      });
      doc.text(
    ...
    const tax = (taxable * quoteDetails.taxRate) / 100;
    const total = taxable + tax;

    const RX = 150,
      VX = 185;

    doc.text("Subtotal:", RX, y, { align: "right" });
    doc.text(`${currency} ${subtotal.toFixed(2)}`, VX, y, { align: "right" });
    y += 6;

    if (discount > 0) {
      doc.setTextColor(200, 0, 0);
      doc.text("Discount:", RX, y, { align: "right" });
      doc.text(`- ${currency} ${discount.toFixed(2)}`, VX, y, {
        align: "right",
      });
      doc.setTextColor(0);
      y += 6;
    ...
    doc.text(`${taxLabel} (${taxRate}%):`, RX, y, { align: "right" });
    doc.text(`${currency} ${tax.toFixed(2)}`, VX, y, { align: "right" });
    y += 8;

    doc.setFontSize(14);
    doc.setFont(undefined, "bold");
    doc.setTextColor(...primaryColor);
    doc.text("Total:", RX, y, { align: "right" });
    doc.text(`${currency} ${total.toFixed(2)}`, VX, y, { align: "right" });
    doc.setFont(undefined, "normal");

    y += 18;

    // NOTES BLOCK
    const block = (title, text, font = 9) => {
      ensureSpace(15);
      doc.setFont(undefined, "bold");
      doc.setFontSize(10);
      doc.setTextColor(0);
      ...
      doc.setFont(undefined, "normal");
      doc.setTextColor(80);
      writeLines(text, 20, 170, font, 5.5);
      y += 6;
    };

    if (quoteDetails.notes)
      block("Notes / Scope of Work:", quoteDetails.notes, 9);
    if (quoteDetails.terms) block("Terms & Conditions:", quoteDetails.terms, 8);
          ...
          <div className="bg-white p-6 rounded-2xl border border-gray-200 shadow-sm space-y-4">
            ...
            <div className="space-y-3">
              ...
              {items.map((item) => (
                <div
                  key={item.id}
                  className="grid grid-cols-1 md:grid-cols-12 gap-3 items-center bg-gray-50 p-2 rounded-xl border border-gray-100 group hover:border-teal-200 transition-colors"
                >
          ...
          <div className="bg-white p-5 rounded-2xl border border-gray-200 shadow-sm">
            ...
            >
              <span className="flex items-center gap-2">
                <Settings size={16} className="text-teal-500" /> Quote Settings
              </span>
            ...
            <div className="space-y-4">
              ...
              <div className="flex justify-between items-center text-gray-600 text-sm">
                <div className="flex items-center gap-2">
                  ...
                  <select
                    name="discountType"
                    value={quoteDetails.discountType}
                    onChange={handleQuoteChange}
              ...
              <div className="flex justify-between items-center text-gray-600 text-sm">
                <div className="flex items-center gap-2">
                  ...
                  <div className="relative w-14">
                    ...
                    />
                    <span className="absolute right-0 top-1 text-xs text-gray-400 pr-1">
                      %
                    </span>
                  </div>
                </div>
